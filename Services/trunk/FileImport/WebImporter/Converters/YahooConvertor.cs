...
ZWD-Zimbabwe Dollar 
        ...
        public override bool DoWork(List<string> soureFilePath, string saveFilePath)
        {
            ...
            CurrencyConvertor.CurrencyConvertor currencyConvertor = new Easynet.Edge.Services.FileImport.CurrencyConvertor.CurrencyConvertor();
                    ...
                    for (int i = 0; i < listTables.Count; i++)
                    {
                        ...
                        try
                        {
                            if (currencyRatioCountries[country] != null)
                            {
                                ...
                                //     Easynet.Edge.Services.FileImport.Converters.
                                //  string gg = rowString;//20091222  91/00/22
                                //    string url = @"http://www.oanda.com/currency/historical-rates?date_fmt=us&date="+rowString.Substring(4,2)+@"/"+rowString.Substring(6,2) +@"/"+rowString.Substring(2,2)  +@"&date1="+ rowString.Substring(4,2)+@"/"+rowString.Substring(6,2) +@"/"+rowString.Substring(2,2)  +@"&exch=" + (Easynet.Edge.Services.FileImport.CurrencyConvertor.Currency)currencyRatioCountries[country] + @"&expr=USD&margin_fixed=0&format=HTML&redirected=1";
                                ...
                                // ConvertionRate = currencyConvertor.ConversionRate((Easynet.Edge.Services.FileImport.CurrencyConvertor.Currency)currencyRatioCountries[country], Easynet.Edge.Services.FileImport.CurrencyConvertor.Currency.USD);
                            }
                        }
                        catch (Exception e)
                        {
