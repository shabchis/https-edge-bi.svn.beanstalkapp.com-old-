...
{
	public partial class AdgroupGateways: BaseAlertActivity
	{

		public AdgroupGateways()
		{
			InitializeComponent();

            _entityType = EntityTypes.Gateway;
		}



        private void LoadGatewaysForAdgroup(int accountID, int channelID, int campaignGK, int adgroupGK)
        {
            using (DataManager.Current.OpenConnection())
            {
                SqlCommand adgroupGateways = BuildCommand();

                adgroupGateways.Parameters["@Campaign_GK"].Value = campaignGK.ToString();
                adgroupGateways.Parameters["@adgroup_GK"].Value = adgroupGK.ToString();

                adgroupGateways.ExecuteNonQuery();
                ...
                while (sdr.Read())
                {
                    GatewayAllMeasures gam = new GatewayAllMeasures(sdr, Filters,AlertMeasures,_alertType);
                    accountName = gam.AccountName;
                    gam.TimeMeasurement = this.TimeMeasurement;
                    gam.MeasurementType = this.MeasurementType;
        ...
        protected override SqlCommand BuildCommand()
        {
            ...
            switch (_alertType)
            {
                case AlertType.Daily:
                    {
                        sql = "SP_Alerts_GatewayPerAdgroupAllMeasuresDayDelta";
                        break;
                    }

                case AlertType.Period:
                    {
                        sql = "SP_Alerts_GatewayPerAdgroupAllMeasuresPeriod";
                        break;
                    }
            }
        ...
        protected override SqlDataReader GetMeasuredData(Hashtable parameters)
        {
            ...
            switch (_alertType)
            {
                case AlertType.Daily:
                    {
                        sql = "SELECT * FROM GatewayPerAdgroupAllMeasuresDayDelta WHERE Account_ID = " + accountID.ToString() + " AND Channel_ID = " + channelID.ToString() + " AND Campaign_gk = " + campaignGK.ToString() + " AND Adgroup_gk = " + adgroupGK.ToString();
                        break;
                    }

                case AlertType.Period:
                    {
                        sql = "SELECT * FROM GatewayPerAdgroupAllMeasuresPeriod WHERE Account_ID = " + accountID.ToString() + " AND Channel_ID = " + channelID.ToString() + " AND Campaign_gk = " + campaignGK.ToString() + " AND Adgroup_gk = " + adgroupGK.ToString();
                        break;
                    }
            }
