...
{
	public partial class AdgroupKeywords: BaseAlertActivity
	{

		public AdgroupKeywords()
		{
			InitializeComponent();

            _entityType = EntityTypes.Keyword;
		}



        private void LoadKeywordsForAdgroup(int accountID, int channelID, int campaignGK, int adgroupGK)
        {
            using (DataManager.Current.OpenConnection())
            {
                ...
                while (sdr.Read())
                {
                    KeywordAllMeasures kam = new KeywordAllMeasures(sdr, Filters, AlertMeasures, _alertType);
                    accountName = kam.AccountName;
                    kam.TimeMeasurement = this.TimeMeasurement;
                    kam.MeasurementType = this.MeasurementType;
        ...
        protected override SqlCommand BuildCommand()
        {
            ...
            switch (_alertType)
            {
                case AlertType.Daily:
                    {
                        sql = "SP_Alerts_KeywordPerAdgroupAllMeasuresDayDelta";
                        break;
                    }

                case AlertType.Period:
                    {
                        sql = "SP_Alerts_KeywordPerAdgroupAllMeasuresPeriod";
                        break;
                    }
            }
        ...
        protected override SqlDataReader GetMeasuredData(Hashtable parameters)
        {
            ...
            switch (_alertType)
            {
                case AlertType.Daily:
                    {
                        sql = "SELECT * FROM KeywordPerAdgroupAllMeasuresDayDelta WHERE Account_ID = " + accountID.ToString() + " AND Channel_ID = " + channelID.ToString() + " AND Campaign_gk = " + campaignGK.ToString() + " AND Adgroup_gk = " + adgroupGK.ToString();
                        break;
                    }

                case AlertType.Period:
                    {
                        sql = "SELECT * FROM KeywordPerAdgroupAllMeasuresPeriod WHERE Account_ID = " + accountID.ToString() + " AND Channel_ID = " + channelID.ToString() + " AND Campaign_gk = " + campaignGK.ToString() + " AND Adgroup_gk = " + adgroupGK.ToString();
                        break;
                    }
            }
        ...
        protected override ActivityExecutionStatus Execute(ActivityExecutionContext executionContext)
        {
            ...
            try
            {
                ...
                if (ParentWorkflow.Parameters.Contains("AdgroupGK") && ParentWorkflow.Parameters.Contains("CampaignGK"))
                {
                    ...
                    LoadKeywordsForAdgroup(accountID, channelID, campaignGK, adgroupGK);
                }
                else
                {
                    ...
                    if (ParentWorkflow.InternalParameters.Contains("AdgroupGKList"))
                    {
                        ...
                        for (int i = 0; i < gks.Count; i++)
                        {
                            ...
                            LoadKeywordsForAdgroup(accountID, channelID, adg._campaignGK, adg._adgroupGK);
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine("Exception at Adgroup Keywords: " + ex.ToString());
                throw ex;
            }
