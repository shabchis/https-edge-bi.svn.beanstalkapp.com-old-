...
namespace EdgeBI.Wizards
{
    ...
    public class WizardRestService
    {
        ...
        [WebInvoke(Method = "POST", UriTemplate = "Previous?sessionID={sessionID}", RequestFormat = WebMessageFormat.Xml, ResponseFormat = WebMessageFormat.Xml)]
        ...
        public ProgressState GetProgress(int sessionID)
        {
            ...
            try
            {
                ...
                {
                    return new ProgressState() { OverAllProgess = 1f};
                }
        ...
        private string GetStateInfo(int sessionID)
        {
            string text=string.Empty;
            using (DataManager.Current.OpenConnection())
            {
                using (SqlCommand sqlCommand = DataManager.CreateCommand(@"SELECT  Top 1  T0.Message                                                                    
                                                                    FROM [Source].[dbo].[Log] T0
                                                                    INNER JOIN Session_Instance_Data T1 ON T0.ServiceInstanceID=T1.InstanceID
                                                                    WHERE T0.IsException=0  AND T1.Session=@Session:Int
                                                                    ORDER BY T0.ID DESC"))
                {
        ...
        public string GetErrors(int sessionID)
        {
            ...
            using (DataManager.Current.OpenConnection())
            {
                using (SqlCommand sqlCommand = DataManager.CreateCommand(@"SELECT       
                                                                     T0.Source                                                          
                                                                     ,T0.Message
                                                                      ,T0.ExceptionDetails
                                                                  FROM Log T0
                                                                    INNER JOIN Session_Instance_Data T1 ON T0.ServiceInstanceID=T1.InstanceID
                                                                    WHERE T0.IsException=1 AND T1.Session=@Session:Int"))
                {
                    sqlCommand.Parameters["@Session"].Value = sessionID;
                    using (SqlDataReader sqlDataReader = sqlCommand.ExecuteReader())
                    {
                        while (sqlDataReader.Read())
                        {
                            logBuilder.AppendLine(string.Format("Source:{0}  \n Message:{1} \n ExceptionDetails:{2}", sqlDataReader[0], sqlDataReader[1], sqlDataReader[2]));
