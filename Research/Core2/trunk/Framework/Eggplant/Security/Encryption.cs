...
bool isClosing)
	...
	public class Encryptor
	{
		...
		public Encryptor(Encoding encoding, string password)
		{
			_encoding = encoding;
			_password = password;

			_key = encoding.GetBytes(password);
			_iv = encoding.GetBytes("@Gjhas1*!6h)6af&3");
			_salt = encoding.GetBytes("8*j1Kl1^79acZyui");
		}
